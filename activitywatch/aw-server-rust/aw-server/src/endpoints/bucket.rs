...
use crate::middleware::api_key::OptionalApiKeyAuth;
use std::env;
...
pub async fn bucket_new(
    ...
    if auth_enabled && auth.0.is_none() {
...
#[get("/<bucket_id>/events/count", rank = 1)]
...
// Note: This route must have lower priority (higher rank) than /events/count to avoid route collision
#[get("/<bucket_id>/events/<event_id>", rank = 2)]
