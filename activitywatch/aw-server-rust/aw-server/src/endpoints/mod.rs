...
pub struct ServerState {
    ...
    pub db_pool: Option<PgPool>, // Database pool for API key operations
}
...
pub fn build_rocket(server_state: ServerState, config: AWConfig) -> rocket::Rocket<rocket::Build> {
    ...
    // Security middleware
    let security_headers = crate::middleware::security::SecurityHeaders;
    let rate_limiter = crate::middleware::rate_limit::RateLimiter::new(100); // 100 requests per minute
    ...
    let mut rocket = rocket::custom(config.to_rocket_config())
        ...
        .mount(
            ...
            routes![
                ...
                bucket::bucket_event_count,  // Must be before bucket_events_get_single to avoid route collision
