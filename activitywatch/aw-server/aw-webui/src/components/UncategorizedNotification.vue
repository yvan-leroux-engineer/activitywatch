...
div
  ...
  b-alert.my-2(v-if="isVisible", variant="info", show)
    p.mb-0
      ...
      br
      | You have a total of {{ uncategorizedTime | friendlyduration }} uncategorized time,
      | that's {{ uncategorizedPercent }}% of all time today.
...
export default {
  ...
  computed: {
    ...
    uncategorizedTime() {
      return this.uncategorizedDuration && this.uncategorizedDuration[0] ? this.uncategorizedDuration[0] : 0;
    },
    uncategorizedPercent() {
      if (!this.uncategorizedDuration || !this.uncategorizedDuration[1]) {
        return 0;
      }
