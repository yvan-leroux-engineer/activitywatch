...
class ActivityWatchClient:
    def __init__(
        ...
        server_protocol = protocol or server_config.get("protocol", "http")
        server_api_key = api_key or server_config.get("api_key", "")
        server_verify_ssl = verify_ssl if verify_ssl is not None else server_config.get("verify_ssl", True)
        
        # Handle boolean string conversion from config
        if isinstance(server_verify_ssl, str):
            server_verify_ssl = server_verify_ssl.lower() in ("true", "1", "yes", "on")
        
        self.server_address = f"{server_protocol}://{server_host}:{server_port}"
        self.verify_ssl = server_verify_ssl
        ...
        # Store API key for session recreation
    ...
    def _delete(self, endpoint: str, data: Any = None) -> req.Response:
        if data is None:
            data = {}
        return self.session.delete(self._url(endpoint), data=json.dumps(data))
    ...
    def disconnect(self):
        ...
        # Restore API key header if it was set
        if self.api_key:
            self.session.headers.update({"X-API-Key": self.api_key})
