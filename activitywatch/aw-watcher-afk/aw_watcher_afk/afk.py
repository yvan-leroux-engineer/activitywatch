...
class AFKWatcher:
    ...
    def heartbeat_loop(self):
        afk = False
        # Store initial parent PID to detect if parent actually died
        # PyInstaller creates a bootloader process, so we can't rely on PPID == 1
        initial_ppid = os.getppid()
        while True:
            try:
                # Check if parent process actually died (only on Unix systems)
                # Skip check for PyInstaller bundles (detected by frozen attribute)
                if system in ["Darwin", "Linux"] and not getattr(sys, 'frozen', False):
                    current_ppid = os.getppid()
                    # Parent died if PPID changed to 1 (init) or changed unexpectedly
                    if current_ppid == 1 and initial_ppid != 1:
