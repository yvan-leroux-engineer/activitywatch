...
func start() {
  ...
  // Check that we get at least 4 arguments (5 with optional API key)
  if arguments.count < 5 || arguments.count > 6 {
    print("Usage: aw-watcher-window <url> <bucket> <hostname> <client> [api_key]")
    exit(1)
  }
  ...
  // API key is optional (5th argument)
  if arguments.count == 6 && !arguments[5].isEmpty {
    apiKey = arguments[5]
    debug("API key provided (length: \(arguments[5].count))")
  } else {
    debug("No API key provided")
  }

  // Check accessibility permissions (check silently first, only prompt if needed)
  let silentOptions: [String: Bool] = [:]
  var hasAccess = AXIsProcessTrustedWithOptions(silentOptions as CFDictionary?)
  
  if !hasAccess {
    // Only prompt once if permissions are not granted
    let checkOptPrompt = kAXTrustedCheckOptionPrompt.takeUnretainedValue() as NSString
    let promptOptions = [checkOptPrompt: true]
    hasAccess = AXIsProcessTrustedWithOptions(promptOptions as CFDictionary?)
    
    if !hasAccess {
      log("Accessibility permissions not granted. Please enable in System Settings > Privacy & Security > Accessibility")
      // Check silently every 30 seconds without prompting again
      DispatchQueue.main.asyncAfter(deadline: .now() + 30) {
        let silentCheck = AXIsProcessTrustedWithOptions(silentOptions as CFDictionary?)
        if silentCheck {
          // Permissions granted, continue initialization
          createBucket()
          setupWatchers()
        } else {
          // Still no permissions, check again silently after 30 seconds
          DispatchQueue.main.asyncAfter(deadline: .now() + 30) {
            let finalCheck = AXIsProcessTrustedWithOptions(silentOptions as CFDictionary?)
            if finalCheck {
              createBucket()
              setupWatchers()
            } else {
              log("Accessibility permissions still not granted. Watcher cannot function without them.")
            }
          }
        }
      }
